...
def group(ctx: GroupMsg):
    if ctx.FromGroupId == qq_group_id and ctx.FromUserId != qq_num:
        if ctx.MsgType == 'TextMsg':
            ...
            tgbot.send_message(
                tg_chat_id, "[QQ]" + " " + ctx.FromNickName + ": " + ctx.Content)
            ...
            tgbot.send_photo(
                tg_chat_id, ctxjs['GroupPic'][0]['Url'], caption="[QQ]" + " " + ctx.FromNickName + pic_text)
...
def echo_all(message):
    ...
    action.sendGroupText(
        qq_group_id, "[TG]" + " " + message.from_user.username + ": " + message.text)
...
def tg_handle_photo(message):
    ...
    pUrl = 'https://api.telegram.org/file/bot{0}/{1}'.format(
        tg_token, file_info.file_path)
    # print(pUrl)
    picfile = requests.get(pUrl, proxies=proxies)
    picfile_name = ".cache/" + fileId
    open(picfile_name, 'wb').write(picfile.content)
    action.sendGroupPic(qq_group_id, picBase64Buf=base_64(
        picfile_name), content="[TG]" + " " + message.from_user.username + pic_text, atUser=0)
